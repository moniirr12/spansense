...
// historicalCheck.js already use; results are cached per element
// for the session so re-hovering a row is instant.
// ============================================================

(function () {
    const SHOW_DELAY = 220;   // ms of hover intent before the cloud appears
    const HIDE_DELAY = 140;   // grace period so moving onto the cloud itself doesn't dismiss it
    const MAX_SHOWN = 3;

    const cache = new Map(); // "structureId:elementNo" -> defects[]
    let showTimer = null;
    let hideTimer = null;
    let activeRow = null;
    let pinned = false;
    ...
    async function fetchDefects(structureId, elementNo) {
    ...
    async function showFor(row) {
        ...
        const defects = await fetchDefects(structureId, elementNo);
    ...
    function scheduleShow(row) {
        ...
        showTimer = setTimeout(function () { showFor(row); }, SHOW_DELAY);
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // open. This fires alongside the existing row-expand click handler —
        // it doesn't interfere with it, it just also surfaces history.
